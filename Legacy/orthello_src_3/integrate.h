...
class WeightLog {
 ...
 public:
  WeightLog(bool __is_black) {
    this->is_black = __is_black;
    this->steps.clear();

    FILE *weight_file;
    if (this->is_black) {
      weight_file = fopen("weight_black", "r");
    } else {
      weight_file = fopen("weight_white", "r");
    }

    for (int i = 0; i < 64; i++) {
      fscanf(weight_file, "%lf", &(weight4[i]));
    }
    fclose(weight_file);
  }
  ...
  void end(Result_t __result) {
    ...
    if (this->is_black && __result == BLACK_WIN) {
      for (Coord_t step : this->steps) {
        weight4[step.first * 8 + step.second] += delta;
      }
    ...
    FILE *weight_file;
    if (this->is_black) {
      weight_file = fopen("weight_black", "w");
    } else {
      weight_file = fopen("weight_white", "w");
    }

    for (int i = 0; i < 8; i++) {
      for (int j = 0; j < 8; j++) {
        fprintf(weight_file, "%lf", (weight4[i*8+j]));
        fprintf(weight_file, " ");
      }
      fprintf(weight_file, "\n");
    }
